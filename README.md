### Howdy 👋 
<p align="left">
  <img src="https://komarev.com/ghpvc/?username=hncleary" alt="hncleary"/>
</p>
<table align="center" style="border-collapse: collapse; width: 100%; max-width: 800px; margin: 0 auto;">
  <tr>
    <td colspan="6" style="padding: 6px; text-align: center; font-size: 0.95em;">
      <b>
      Public Projects
    </b>
    <br>
      <span style="font-size: 0.85em; color: #666;">
        Personal mini-projects currently published online
      </span>
    </td>
  </tr>
  <tr style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <img src="./assets/images/grayson-grappler.png" style="width:35px; height: auto; max-height: 35px; object-fit: contain;">
      </td>
      <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
        <div style="font-size: 0.95em; margin: 0;">
          <b>
          Cavern Collapse
        </b>
      </div>
      <div style="font-size: 0.85em; color: #666; margin-top: 2px;">
        High score arcade survival game in a collapsing cavern
      </div>
    </td>
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 4px 8px; align-items: center;">
        <a href="https://cyranek.itch.io/cavern-collapse">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/itchio.png" />
          itch.io
        </a>
        <a href="https://www.newgrounds.com/portal/view/860822">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/newgrounds.png" />
          Newgrounds
        </a>
        <a href="https://apps.apple.com/us/app/cavern-collapse/id6451268649">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/apple.png" />
          iOS
        </a>
        <a href="https://play.google.com/store/apps/details?id=cyranek.com.cyranek.caverncollapse&hl=en_US&gl=US">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/android.svg" />
          Android
        </a>
      </div>
    </td>
  </tr>
  <tr style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <img src="./assets/images/void_miner.png" style="width:35px; height: auto; max-height: 35px; object-fit: contain;">
      </td>
      <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
        <div style="font-size: 0.95em; margin: 0;">
          <b>
          Void Miner
        </b>
      </div>
      <div style="font-size: 0.85em; color: #666; margin-top: 2px;">
        Infinite idle asteroid mining
      </div>
    </td>
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 4px 8px; align-items: center;">
        <a href="https://cyranek.itch.io/void-miner">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/itchio.png" />
          itch.io
        </a>
        <a href="https://www.newgrounds.com/portal/view/1007771">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/newgrounds.png" />
          Newgrounds
        </a>
      </div>
    </td>
  </tr>
  <tr style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <img src="./assets/images/ski-freak.png" style="width:35px; height: auto; max-height: 35px; object-fit: contain;">
      </td>
      <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
        <div style="font-size: 0.95em; margin: 0;">
          <b>
          Ski Freak
        </b>
      </div>
      <div style="font-size: 0.85em; color: #666; margin-top: 2px;">
        One tap endless skiing arcade game
      </div>
    </td>
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 4px 8px; align-items: center;">
        <a href="https://cyranek.itch.io/ski-freak">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/itchio.png" />
          itch.io
        </a>
        <a href="https://www.newgrounds.com/portal/view/1023135">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/newgrounds.png" />
          Newgrounds
        </a>
        <a href="https://store.steampowered.com/app/3664680/Ski_Freak">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/steam.png" />
          Steam
        </a>
      </div>
    </td>
  </tr>
  <tr style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <img src="./assets/images/jumper_logo.png" style="width:35px; height: auto; max-height: 35px; object-fit: contain;">
      </td>
      <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
        <div style="font-size: 0.95em; margin: 0;">
          <b>
          Jumper
        </b>
      </div>
      <div style="font-size: 0.85em; color: #666; margin-top: 2px;">
        High score hopper arcade game
      </div>
    </td>
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 4px 8px; align-items: center;">
        <a href="https://store.steampowered.com/app/5050840/Jumper/">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/steam.png" />
          Steam
        </a>
      </div>
    </td>
  </tr>
  <tr style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <img src="./assets/images/ch8t_gpt_logo.png" style="width:35px; height: auto; max-height: 35px; object-fit: contain;">
      </td>
      <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
        <div style="font-size: 0.95em; margin: 0;">
          <b>
          ch8t-GPT
        </b>
      </div>
      <div style="font-size: 0.85em; color: #666; margin-top: 2px;">
        The world's most powerful LLM AI chat bot powered by divination
      </div>
    </td>
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 4px 8px; align-items: center;">
        <a href="https://ch8tgpt.com/">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/globe.png" />
          Site
        </a>
        <a href="https://github.com/hncleary/eight-ball-gpt">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/github.png" />
          Source
        </a>
      </div>
    </td>
  </tr>
  <tr style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <img src="./assets/images/vps-logo.png" style="width:35px; height: auto; max-height: 35px; object-fit: contain;">
      </td>
      <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
        <div style="font-size: 0.95em; margin: 0;">
          <b>
          Vanity Plate Social
        </b>
      </div>
      <div style="font-size: 0.85em; color: #666; margin-top: 2px;">
        Consolidated retrieval and display of social media stats
      </div>
    </td>
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 4px 8px; align-items: center;">
        <a href="https://www.vanityplate.social/home">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/globe.png" />
          Site
        </a>
        <a href="https://github.com/hncleary/vanity-plate-ng">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/github.png" />
          Site Source
        </a>
        <a href="https://github.com/hncleary/vanity-plate-pr">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/github.png" />
          Retrieval
        </a>
      </div>
    </td>
  </tr>
  <tr style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <img src="./assets/images/duuzu_db_logo.png" style="width:35px; height: auto; max-height: 35px; object-fit: contain;">
      </td>
      <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
        <div style="font-size: 0.95em; margin: 0;">
          <b>
          Duuzu Key BPM DB
        </b>
      </div>
      <div style="font-size: 0.85em; color: #666; margin-top: 2px;">
        Searchable song key and bpm database website for DJs
      </div>
    </td>
    <td style="padding: 4px 8px; font-size: 0.9em; line-height: 1.3;">
      <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 4px 8px; align-items: center;">
        <a href="https://black-water-0be05fd10.5.azurestaticapps.net/home">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/globe.png" />
          Site
        </a>
        <a href="https://github.com/hncleary/duuzu-key-bpm-db">
          <img style="width:16px; height: auto; max-height: 16px; vertical-align: middle; margin-right: 4px;" src="./assets/icons/github.png" />
          Source
        </a>
      </div>
    </td>
  </tr>
</table>